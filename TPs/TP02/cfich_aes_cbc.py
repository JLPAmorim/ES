...
def encrypt(input='fich.txt', fkey='fkey', output='fich.enc'):
    ...
    # Usa a chave e o IV para encriptar o ficheiro pretendido no modo CBC
    cipher = Cipher(algorithms.AES(key), modes.CBC(iv), backend=default_backend())
    encryptor = cipher.encryptor()
    ciphertext = encryptor.update(padded_plaintext) + encryptor.finalize()

    # Escreve o IV e o criptograma para o ficheiro fich.enc
...
def decrypt(input='fich.enc', fkey='fkey', output='fich.dec'):
    ...
    # Determina o tamanho do IV
    iv_length = 16

    # Lê o ficheiro para decifrar
    with open(input, 'rb') as input_file:
        iv = input_file.read(iv_length)
        ciphertext = input_file.read()

    # Decifra o criptograma usando a chave e o IV no modo CBC
    cipher = Cipher(algorithms.AES(key), modes.CBC(iv), backend=default_backend())
