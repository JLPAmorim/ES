...
function setup(fkey) {
  ...
  fs.writeFileSync(`./cfich_aes_ctr/${fkey}`, key)
...
function decrypt(fich, fkey) {
  ...
  const key = fs.readFileSync(`./cfich_aes_ctr/${fkey}`)
  const content = fs.readFileSync(`./cfich_aes_ctr/${fich}`)
  ...
  fs.writeFileSync(`./cfich_aes_ctr/${fich}`.replace(".enc", ".dec"), decrypted)
}
...
switch (args[0]) {
  ...
  default:
    ...
    console.log()
    console.log("node cfich_aes_ctr.js setup fkey")
    console.log("node cfich_aes_ctr.js enc fich fkey")
    console.log("node cfich_aes_ctr.js dec fich fkey")
}
